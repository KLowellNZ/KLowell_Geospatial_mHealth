...
# KPI for all credit unions, then output all 96 KPIs for the 6000ish credit
...
fspath='C:/Analytics/DATA911/Arkatechture/NCUA_Data/NCUA_Data_Final/'
outpath='C:/Analytics/DATA911/Arkatechture/NCUA_Data/NCUA_Data_Final/NCUA_KPIs/'
#prefix='201703_'
prefix='201706_'
#prefix='201612_'
#prefix='201512_'
...
dfout=pd.read_csv(fspath+prefix+callreport+prefix+'foicu.csv',
      usecols=['CU_NUMBER','JOIN_NUMBER','CU_NAME','CITY','STATE','ZIP_CODE'],
      skipinitialspace=True)
outfile=outpath+prefix+'KPIs.csv'
...
# Infile 1 is the file that says which ACCTs are used to calculate which KPIs.
...
# Dictionary now established.  Set up dictionary so KPI names are a bit
# more descriptive.
...
# Get ACCTs that migth be used in searches.
acctlist=['acct_997','acct_010','acct_025b','acct_018','acct_131']
dfaccts=KPIdfs(acctlist,acctdict)
dfaccts=dfaccts.rename(columns = {'ACCT_997':'NetWorth','ACCT_010':'Assets',
                       'ACCT_025B':'LoansLeases','ACCT_018':'DeposShars',
                       'ACCT_131':'FeeInc'})
dfout=pd.merge(left=dfout, right=dfaccts,how='left')
# Now Get KPIs and attach them to the output file.
# NOTE: The specific call report Quarter/Year is set in KPIdfs.CHANGE THIS
...
dfout=JoinKPI(KPInum,dfout,dfaccts,KPIclass,KPInamedict)
#......................................................
# KPI 10: Return on Average Assets - Required PYE/AC not used
#  661A, 0101
##########  ADAPT FOR PYE  ############################
KPInum=10
KPIclass='Earnings'
...
acctlist=['acct_703','acct_386','acct_712','acct_400t','acct_814e','acct_799c1',
          ...
          'acct_010']
# This KPI did not exist prior to 2013. Put in error traps.
try:
    dfaccts=KPIdfs(acctlist,acctdict)
# Protect against division by zero -- drop rows if denominator = 0.
    dfaccts = dfaccts.drop(dfaccts[dfaccts['ACCT_010'] == 0].index)
